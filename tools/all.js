...
// let ks_id = 'NWI5NDMTA1MjcyMDc0NjUxMDAxTmVZMDRJVTJQ'
const metas = {
  appName: 'melody',
  appVersion: '4.4.0',
  ksid: ks_id
...
instance.interceptors.response.use(
  res => {
    ...
    if (res.data.code === 0) {
      ...
      if (res.data?.msg == '请勿重复提交' || res.data?.msg == '服务器超时，请稍后再试') {
        return new Promise(resolve =>
          setTimeout(() => resolve(instance({ ...res.config })), 300)
        )
      }
  ...
  error => {
    ...
    // console.log(error)
    const shouldRetry =
      (/ETIMEDOUT|ECONNRESET|ECONNABORTED|ENOTFOUND/.test(error.code)) &&
    ...
    if (shouldRetry) {
      ...
      return new Promise(resolve =>
        setTimeout(() => resolve(instance({ ...config })), 300)
      )
    }
...
async function a(wmPoiId) {
  try {
    ...
    // let shops = await execRequest(instanceElm, y.requests.elm['所有门店/get'], {}, xshard(93089700))
    // shops.map(async shop => {
    //   let {activities} = await execRequest(instanceElm, y.requests.elm['活动列表/get'], [shop.id], xshard(shop.id))
    //   let act = activities.find(a=>a.title= "超级吃货红包")
    //   return {shopId: shop.id, actId: act}
    // })

    // console.log(shops)
    // let data = await readXls('plan/美团改产品名.xlsx', 'Sheet1')
    // data = data.map(v => [v.wmpoiid, v.productId])
    // await loop(ddbs, data)

    // let shops = await knx('foxx_shop_reptile').where({status: 0}).andWhere('reptile_type', 'like', '%甜品%')
    // let data = shops.map(v => [v.wmpoiid])
    // await loop(xdfq, data)

    // let data = await readXls('plan/收藏有礼(1).xlsx', 'hh变化查询美团')
    // data = data.map(v => [v.wmpoiid])
    // await loop(scyl, data)
...
async function scyl(wmPoiId) {
  try {
    const { list } = await execRequest(undefined, y.requests.mt['收藏有礼/get'], [wmPoiId])
    const act = list.find(v => v.status == 1)
    if (act) {
      // console.log(act)
      // return execRequest(undefined, y.requests.mt['新客立减/disable'], [wmPoiId, act.id])
      return execRequest(undefined, y.requests.mt['收藏有礼/update'], [[wmPoiId], act.activityId])
      // await sleep(3000)
    }
...
async function xklj(wmPoiId) {
  try {
    const { list } = await execRequest(undefined, y.requests.mt['店铺活动/get'], [wmPoiId, '1,8'])
    const act = list.find(v => v.name == '新用户立减')

    if (act) {
      return execRequest(undefined, y.requests.mt['新客立减/disable'], [wmPoiId, act.id])
    }

    // const { list } = await execRequest(undefined, y.requests.mt['新客立减/get'], [wmPoiId])
    // const act = list.find(v => v.status == 1)
    // if (act) {
    //   // console.log(act)
    //   return execRequest(undefined, y.requests.mt['新客立减/disable'], [wmPoiId, act.id])
    //   // await sleep(3000)
    // }
    // return execRequest(undefined, y.requests.mt['test/下单返券/create'], [wmPoiId, date(), date(365)])
...
async function xdfq(wmPoiId) {
  try {
    const { list } = await execRequest(undefined, y.requests.mt['下单返券/get'], [wmPoiId])
    const act = list.find(v => v.status == 1)
    if (act) {
      // console.log(act)
      await execRequest(undefined, y.requests.mt['下单返券/delete'], [wmPoiId, act.activityId])
      await sleep(3000)
    }
    return execRequest(undefined, y.requests.mt['test/下单返券/create'], [wmPoiId, date(), date(365)])
...
async function ddbs(wmPoiId, spuIds) {
  try {
    return execRequest(undefined, y.requests.mt['单点不送/update'], [wmPoiId, spuIds])
  } catch (e) {
    return Promise.reject(e)
  }
}
  ...
  try {
    ...
    const allTasks = [
        ...
        fn: async function () {
          try {
            ...
            return Promise.allSettled(acts.map(item => limit(async () => {
              ...
              if (!food) {
                console.error('act : name not matched', item.itemName)
                return Promise.reject(JSON.stringify({ name: item.itemName, reason: '商品不存在' }))
              }
              ...
              console.log('create req...', policy)

              const r = await execRequest(undefined, y.requests.mt['折扣商品/create'], [wmPoiId, [policy]])
              console.log(item.itemName, r)
              return Promise.resolve({ name: item.itemName, r })
            })))
          } catch (e) {
            return Promise.reject(e)
          }
      ...
      },
      {
        ...
        fn: async function () {
          try {
            const limit = pLimit(1), limitB = pLimit(50)
            let { tagList } = await execRequest(undefined, y.requests.mt['分类列表/get'], [wmPoiId])

            return Promise.allSettled(tagList.map(tag => limit(async () => {
              let { productList } = await execRequest(undefined, y.requests.mt['商品列表/get'], [wmPoiId, tag.id])

              const foodsRes = await execRequest(undefined, y.requests.mt['商品删除/delete'], [wmPoiId,
                productList.flatMap(food => food.wmProductSkus.map(sku => sku.id)).join(',')
              ])

              console.log(foodsRes)
              const tagRes = await execRequest(undefined, y.requests.mt['分类删除/delete'], [wmPoiId, tag.id])
              return { tag: tag.name, tagRes, foodsRes }
            })))
          } catch (e) {
            return Promise.reject(e)
          }
...
async function testsDel(wmPoiId) {
  const fallbackApp = new FallbackApp(wmPoiId, singleCookie)

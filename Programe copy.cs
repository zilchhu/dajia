    ...
    public static async Task BuildTable1()
    {
      ...
      foreach (var v in data)
      {
        ...
        row.Append(
          ...
          new Cell { CellReference = $"I{dataIndex}", DataType = CellValues.Number, CellValue = new CellValue(v.Cost_Sum_Ratio) },
          ...
          new Cell { CellReference = $"M{dataIndex}", DataType = CellValues.Number, CellValue = new CellValue(v.Consume_Sum_Ratio) },
      ...
      var pt = new PivotTableDefinition
      {
        ...
        MinRefreshableVersion = 3,
        DataCaption = "Values",
        Location = new Location { Reference = $"A3:{toColName(dates.Count() + 2 - 1)}{persons.Count() + 1 + 3 + 2}", FirstHeaderRow = 1, FirstDataRow = 2, FirstDataColumn = 1 },
        PivotFields = pfs,
        RowFields = rfs,
        RowItems = ris,
        ColumnFields = cfs,
